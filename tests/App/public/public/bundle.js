...
function Marker(){function constructor(paper,props){this.paper=paper;this.props=props;this.circle={};this.ring={};this.text={};this.positions={};this.interval=null;var opacity=props.opacity,radius=props.radius;if(!opacity){this.opacity=Marker.prototype.OPACITY;}else{this.opacity=opacity;}if(!radius){this.radius=Marker.prototype.RADIUS;}else{this.radius=radius;}return this;}return constructor.apply(this,arguments);}Marker.prototype={OPACITY:0.4,RADIUS:10,placeMarker:function placeMarker(x,y,color,Dest){this.positions[color]={'x':x,'y':y};this.circle[color]=this.paper.circle(x,y,this.radius).attr({fill:color,opacity:this.opacity});this.text[color]=this.paper.text(x+this.offset(),y-this.offset(),Dest).attr({fontSize:'13px',fill:'#000',stroke:'#000'});},removeMarker:function removeMarker(color){this.circle[color].remove();this.text[color].remove();},offset:function offset(){return 3;},isPointInside:function isPointInside(x,y){var cx=null,cy=null,present=false;for(var color in this.circle){cx=parseFloat(this.circle[color].attr('cx'));cy=parseFloat(this.circle[color].attr('cy'));if(Math.sqrt(Math.pow(cx-x,2)+Math.pow(cy-y,2))<=this.radius){present=true;break;}}return present;},animateMarker:function animateMarker(color){var instance=this;this.ring[color]=this.paper.circle(this.positions[color]['x'],this.positions[color]['y'],this.radius+2).attr({fill:"transparent",opacity:1.0});this.interval=window.setInterval(function(){instance.circle[color].animate({opacity:Marker.prototype.OPACITY},1500,Snap.mina,function(){instance.circle[color].animate({opacity:1.0},1000);});},1500);},clearMarker:function clearMarker(){window.clearInterval(this.interval);}};module.exports=Marker;
